...
// Helper function to get token from URL, handling potential URL encoding issues
function getTokenFromUrl() {
  // Get from URL search params
  const urlParams = new URLSearchParams(window.location.search)
  const token = urlParams.get('token')
  
  // If token contains a space or newline, clean it
...
async function handleResetPassword() {
  ...
  try {    
    ...
    // First determine which flow we're using
    const isJwtFlow = !!token
    const isSupabaseFlow = !!(hash && hash.includes('type=recovery'))
    
    if (!isJwtFlow && !isSupabaseFlow) {
      throw new Error('Password reset token is missing. Please check the reset link or request a new one.')
    }
    
    // Clean the token to remove any whitespace or line breaks that might have been added
    const cleanToken = token ? token.replace(/\s/g, '') : null
    
    // Try the appropriate flow for password reset
    if (isJwtFlow) {
      // Use our backend API for JWT-based reset
      console.log('Using backend API for JWT-based password reset')
      ...
      console.log('Password reset successful via backend JWT flow')
      ...
      if (resetError) {
        // If Supabase direct update fails, try using our backend API as fallback
        console.log('Supabase reset failed, trying backend fallback')
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/auth/update-password`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
        ...
        console.log('Password reset successful via backend (Supabase fallback)')
        toast.info('Password has been reset!')
      } else {
        console.log('Password reset successful via direct Supabase update')
...
onMounted(async () => {
  ...
  // First check for token in query params (our custom JWT token)
  const token = getTokenFromUrl()
  console.log('Reset token from URL:', token ? `${token.substring(0, 10)}...` : 'not found')
  
  // If no active session and no token in query params, check hash for Supabase flow
