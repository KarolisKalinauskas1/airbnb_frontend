...
import { checkApiAvailability, getApiHealthState } from '@/utils/enhancedHealthCheck';
...
const checkHealth = async (force = false) => {
  const isHealthy = await checkApiAvailability(force);
  
  // Only show banner for actual API issues (not network issues)
  if (!isHealthy && getApiHealthState().consecutiveFailures > 1) {
...
const initializeComponent = async () => {
  try {
    ...
    console.error('Error during network status initialization:', error);
  }
};
