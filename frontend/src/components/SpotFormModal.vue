...
const removeExistingImage = async (imageId) => {
  ...
  if (confirm('Are you sure you want to delete this image?')) {
    try {
      ...
      // Update the local state and emit an event to update parent component
      if (props.spot.images) {
        props.spot.images = props.spot.images.filter(img => img.image_id !== imageId);
        emit('spot-updated', props.spot);
      }
...
const handleSubmit = async () => {
  ...
  if (isSubmitting.value || submissionLock.value) return
  
  // Lock submissions until the process completes
    ...
    let response;      if (props.spot) {
      ...
      try {
        ...
        if (error.response?.status === 401) {
          ...
          try {
            await authStore.refreshToken();            const retryToken = await authStore.getAuthToken();
            response = await axios.put(`/api/camping-spots/${props.spot.camping_spot_id}`, formData, {
              headers: {
                'Content-Type': 'multipart/form-data',
...
onMounted(async () => {
  ...
  if (props.spot) {
    ...
    selectedAmenities.value = props.spot.camping_spot_amenities?.map(a => typeof a.amenity_id === 'number' ? a.amenity_id : parseInt(a.amenity_id)) || [];
  }
});
onUnmounted(() => {
  isSubmitting.value = false;
