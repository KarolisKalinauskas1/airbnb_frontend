...
const fetchAmenities = async () => {
  ...
  if (!shouldAllowRequest('/amenities')) {
    return;
  }
  try {
    ...
    await runWithLoading(async () => {
      ...
      let response;
      try {
        response = await axios.get('/amenities', { 
          timeout: 5000,
  ...
  } catch (error) {    loadingError.value = error.message || 'Failed to load amenities';
    console.error('Error fetching amenities:', error);
    
    // Try alternative endpoint if the first attempt failed
    try {
      const fallbackResponse = await axios.get('/camping-spots/amenities', { 
        timeout: 5000,
        headers: { 'Accept': 'application/json' } 
      });
      
      if (fallbackResponse && fallbackResponse.data && Array.isArray(fallbackResponse.data)) {
        amenitiesData.value = fallbackResponse.data;
        loadingError.value = null; // Clear error if fallback succeeded
